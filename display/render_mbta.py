...
def render_mbta_content(display: Any, message: RenderMessage.MBTA) -> None:
    ...
    if status in [
        ...
        for i, p in enumerate(predictions):
            draw.text((0, i * 16), p.label, font=Fonts.MBTA, fill=Colors.MBTA_AMBER)
            draw.text(
                (display.SCREEN_WIDTH, i * 16),
                p.value,
                font=Fonts.MBTA,
...
def render_mbta_banner_content(display: Any, message: RenderMessage.MBTABanner) -> None:
    ...
    if display.last_mbta_image is not None:
        animations["mbta_content_scroll_away"] = MoveAnimation(
            Rect(0, 0, display.SCREEN_WIDTH, display.SCREEN_HEIGHT),
            Rect(
                0, -display.SCREEN_HEIGHT, display.SCREEN_WIDTH, display.SCREEN_HEIGHT
            ),
            display.last_mbta_image,
            speed=60,
            loop=False,
        )
